 /* SKILL: Densification (Cross Join + Aggregation in one step)
    Grid is built from a small distinct-bank lookup (5 rows) cross joined
    with the 3 buckets, rather than cross joining the full raw table.
*/
...
Banks AS (
    -- 2a. Distinct bank list (5 rows), not the full fact table
    SELECT DISTINCT Issuer_Bank
    FROM `project-43c16c81-2fd4-4871-8ac.payment_optimization.payments`
),
Grid AS (
    -- 2b. Create the perfect 15-row grid (5 Banks * 3 Buckets)
    SELECT Issuer_Bank, Bucket_Name
    FROM Banks
    CROSS JOIN (
